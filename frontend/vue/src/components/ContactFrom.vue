...
<template>
    <section>
      <div class="mt-[112px] flex justify-center">
        <h2 class="text-2xl text-[#1A2B4B]">Готовы вывести бизнес на новый уровень?</h2>
      </div>
      <div class="flex justify-center mt-8">
      <form @submit.prevent="sendForm" class="space-y-4 w-[512px] ">
        <div>
        ...
        <div class="flex gap-4">
          <input v-model="email" placeholder="E-mail" class="flex-1 bg-white placeholder:text-[#1A2B4B] border border-[#D1D5DB] px-4 py-3 focus:outline-none focus:border-[#a3aab3] focus:ring-0 rounded"/>
          <button type="submit" class="text-[#FFFFFF] bg-[#1D65C1] max-w-[195px] max-h-[50px] px-8 py-[13px] active:scale-95 transition-transform cursor-pointer rounded">Оставить заявку</button>
        </div>
