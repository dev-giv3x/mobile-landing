...
const getIcon = (name?: string | null) => {
  return name ? icons[`../assets/icons/${name}.svg`] ?? '' : ''
}

const modules = ref<LandingModule[]>([])

onMounted(async () => {
  const response = await axios.get<LandingModule[]>('http://localhost:80/api/modules')
...
<template>
  <section class="px-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12 max-w-[1120px] mx-auto justify-items-center">
      <div class="w-full max-w-[357px] h-[192px] bg-white rounded-xl shadow-sm border border-gray-50 flex flex-col" v-for="module in modules" :key="module.id">
        <div class="flex items-center gap-3 px-[25px] pt-[25px]">
          <img :src="getIcon(module.primary_icon)" class="w-8 h-8 object-contain" alt="" />
          <h3 class="text-lg text-[#1A2B4B]">{{ module.title }}</h3>
        </div>
        <div class="w-[307.33px] h-px bg-[#F3F4F6] ml-[25px] mt-4 mb-3"></div>
        <p class=" text-base text-[#5F738C] pl-[25px]"> {{ module.content }} </p>
        <div class="w-[307.33px] h-px bg-[#F3F4F6] ml-[25px] mt-4 mb-3"></div>
        <div class="flex items-center gap-3 px-[25px] pb-[20px]">
          <img :src="getIcon(module.secondary_icon)" class="w-5 h-5 object-contain" alt="" />
          <p class="text-sm text-[#5F738C]">{{ module.secondary_content }}</p>
        </div>
      </div>
    </div>
  </section>
